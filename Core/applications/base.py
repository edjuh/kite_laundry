...
# === External utilities ===
from utils.physics import (calculate_difficulty,  # 👈 new physics link
                           calculate_drag)
...
class GeometryLib:
    ...
    def add_seam_allowance(
        points: List[Tuple[float, float]], allowance: float = 10
...
def generate_cone_geometry(project, length_mm, settings):
    ...
    gore_points = [(0, 0), (top_width, 0), (bottom_width, height), (0, height)]
...
def generate_cylinder_geometry(project, length_mm, settings):
    ...
    segment_points = [(0, 0), (panel_width, 0), (panel_width, height), (0, height)]
...
def normalize_project_dict(
    project: Dict[str, Any], filename: Optional[str] = None
    ...
    if not isinstance(proj.get("parameters"), dict):
        ...
        geom_block = (
            project.get("geometry") if isinstance(project.get("geometry"), dict) else {}
        )
    ...
    gf = (
        project.get("geometry_fn")
        if isinstance(project.get("geometry_fn"), str)
        else None
    )
    if not geom_type and gf:
        ...
        if (
            ...
            or "drogue" in gf_l
            or "chute" in gf_l
            or "parachute" in gf_l
        ):
            geom_type = "cone"
        elif (
            "spinner" in gf_l or "tube" in gf_l or "cylinder" in gf_l or "blimp" in gf_l
        ):
            geom_type = "cylinder"
    ...
    if not isinstance(proj.get("colors"), dict):
        ...
        cp = (
            project.get("color_pattern")
            or project.get("colors")
            or project.get("palette")
        )
    ...
    def calculate_requirements(self, panels: List[Dict], project: Dict = None) -> Dict:
        ...
        if project and "materials" in project:
            ...
            if isinstance(materials, dict):
                material_type = materials.get(
                    "fabric", materials.get("thread", "Unknown")
                )
...
class PreviewGenerator:
    ...
    def generate_preview(project: Dict, panels: List[Dict], out_dir: str) -> str:
        ...
        svg.append(
            f'<text x="400" y="30" text-anchor="middle" font-size="18" fill="{title_color}">{project["name"]}</text>'
        )
        ...
        for i, p in enumerate(panels):
            ...
            svg.append(
                f'<polygon points="{pts}" fill="{color}" stroke="black" opacity="0.6"/>'
            )
...
def run_pipeline(project: Dict, length_mm: float, out_dir: str, settings: Dict) -> Dict:
    try:
        ...
        doc_path = HTMLDocGenerator.generate_documentation(
            project, panels, materials, out_dir
        )
