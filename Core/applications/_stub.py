...
def simple_preview(out_dir, title="Preview", color="#888"):
    previews = os.path.join(out_dir,'previews'); os.makedirs(previews, exist_ok=True)
    fig, ax = plt.subplots(figsize=(4,6))
    ax.set_xlim(-1,1); ax.set_ylim(0,1.5)
    ax.axis('off')
    ax.add_patch(patches.Ellipse((0,0.7),0.8,1.2,facecolor=color, edgecolor='black'))
...
def simple_labeled_preview(out_dir, title="Preview", labels=None):
    ...
    ax.set_xlim(-1,1); ax.set_ylim(0,1.4)
    ax.axis('off')
    ax.add_patch(patches.Ellipse((0,0.65),0.8,1.1,facecolor="#222"))
    if labels:
        for name, (lx,ly,px,py,color,txt) in labels.items():
            ax.annotate('', xy=(px,py), xytext=(lx,ly), arrowprops=dict(arrowstyle='-', lw=1.0, color=color))
            ax.text(lx,ly, f"{name} — {txt}", ha='center', va='center', fontsize=7, color='white',
...
def bol_tiling(project, out_dir, settings, num_gores=12):
    ...
    pdf_path = os.path.join(templates_dir, f"{project['name'].replace(' ','_')}_Tiled.pdf")
    ...
    for r in range(rows):
        for col in range(cols):
            # crop marks
            c.setLineWidth(0.5)
            c.line(0,10,0,30); c.line(0,page_h-30,0,page_h-10)
            c.line(page_w-0,10,page_w-0,30); c.line(page_w-0,page_h-30,page_w-0,page_h-10)
            c.line(10,0,30,0); c.line(page_w-30,0,page_w-10,0)
            c.line(10,page_h-0,30,page_h-0); c.line(page_w-30,page_h-0,page_w-10,page_h-0)
            ...
            # seam allowance
            c.setDash(3,2)
            c.roundRect(x0-10, y0-10, gore_width+20, gore_height+20, 30, stroke=1, fill=0)
            c.setDash()
